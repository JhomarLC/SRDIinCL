<script>
    ...
    $(document).ready(function () {
        ...
        async function finalValidationBeforeSubmit(formData) {
            try {
                await $.ajax({
                    url: "{{ route('speaker-eval.validateAll') }}", // You can also use route('participant.validateStep') in sequence
                    method: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        const submitFinalForm = () => {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    setTimeout(() => {
                        window.location.href = "{{ url('/speaker-management/' . $speaker->id . '/topics/' . $speaker_topic->id) . '/evaluations'}}";
                    }, 1500);
                },
                error: function (xhr) {
    ...
    $(document).ready(function () {
        ...
        $("#unarchiveEvalForm").submit(function (e) {
            ...
            $.ajax({
                url: "/speaker-management/" + speakerId + "/topics/" + topicId + "/evaluations/" + evalId + '/unarchive',
                type: "PUT",
                data: formData,
                dataType: "json",
                success: function (response) {
        ...
        $("#archiveEvalForm").submit(function (e) {
            ...
            $.ajax({
                url: "/speaker-management/" + speakerId + "/topics/" + topicId + "/evaluations/" + evalId + '/archive',
                type: "PUT",
                data: formData,
                dataType: "json",
                success: function (response) {
